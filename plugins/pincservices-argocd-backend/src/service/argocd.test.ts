...
describe('ArgoCD service', () => {
    ...
    it('should sync app', async () => {
        ...
        const resp = await argoService.syncArgoApp({
            argoInstance: {
                name: 'testApp',
                url: 'https://argoInstance1.com',
                appNameAndNamespaces: [{name: 'testApp', appNamespace: 'argocd'}],
            },
            argoToken: 'testToken',
            appName: 'testApp',
            appNamespace: 'argocd'
    ...
    describe('findArgoApp', () => {
        it('should return the argo instances an argo app is on', async () => {
            ...
            const resp = await argoService.findArgoApp({ name: 'testApp-nonprod', appNamespace: 'argocd' });

            expect(resp).toStrictEqual([
                {
                    name: 'argoInstance1',
                    url: 'https://argoInstance1.com',
                    appNameAndNamespaces: [{name: 'testApp-nonprod', appNamespace: 'argocd'}],
                },
            ]);
        });
