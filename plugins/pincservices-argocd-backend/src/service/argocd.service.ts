...
export class ArgoService implements ArgoServiceApi {
    ...
    async getRevisionData(
        ...
        const query = this.getQueryParams({ appNamespace: options.appNamespace });
        ...
        const resp = await fetch(
            `${baseUrl}/api/v1/applications/${options.name}/revisions/${revisionID}/metadata${query}`,
            requestOptions,
        );
        ...
        const resp = await Promise.all(
            this.instanceConfigs.map(async (argoInstance: any) => {
                ...
                return {
                    ...
                    appNameAndNamespaces: options.selector
                        ? getArgoAppDataResp.items.map((x: any) => {return {name: x.metadata.name, appNamespace: x.metadata.namespace};})
                        : [{name: options.name, appNamespace: options.appNamespace}],
                };
            }),
        ).catch();
    ...
    async getArgoAppData(
        ...
        const query = this.getQueryParams({ selector: options?.selector, appNamespace: options?.appNamespace });
        let urlSuffix = '';
        if (options?.name) {
            urlSuffix = `/${options.name}`;
        }

        urlSuffix = `${urlSuffix}${query}`;
            ...
            const parallelSyncCalls = argoAppResp.map(
                async (argoInstance: any): Promise<SyncResponse[]> => {
                    try {
                        ...
                        try {
                            const resp = argoInstance.appNameAndNamespaces.map(
                                (argoApp: any): Promise<SyncResponse> => {
                                    return this.syncArgoApp({
                                        argoInstance,
                                        argoToken: token,
                                        appName: argoApp.name,
                                        appNamespace: argoApp.appNamespace
                                    });
                                },
                            );
